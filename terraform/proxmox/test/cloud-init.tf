data "template_file" "cloud_init_test_template" {
  count    = var.vm_count
...
resource "local_file" "cloud_init_test_local" {
  count    = var.vm_count
  content  = element(data.template_file.cloud_init_test_template.*.rendered, count.index)
  filename = "${path.module}/files/cloud_init_${var.proxmox_vm_name}-${count.index+1}.cfg"
}

resource "null_resource" "cloud_init_test" {
  count = var.vm_count
  connection {
    type        = "ssh"
  ...
  provisioner "file" {
    source      = element(local_file.cloud_init_test_local.*.filename, count.index)
