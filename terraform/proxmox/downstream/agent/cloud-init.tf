resource "proxmox_cloud_init_disk" "agent_ci" {
  name     = "${var.proxmox_vm_name}-agent-${count.index + 1}-cloud-init"
...
EOT
  ...
  meta_data = <<EOT
instance_id: ${sha1("${var.proxmox_vm_name}-agent-${count.index + 1}")}
EOT
...
fqdn: ${var.proxmox_vm_name}-agent-${count.index + 1}.${var.cloud_init_domain}
write_files:
